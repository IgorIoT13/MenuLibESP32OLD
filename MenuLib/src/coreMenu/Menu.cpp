...
void printScreen (Menu* menu){
    ...
    if(screen && lcd){
        ...
        lcd->print("");
        lcd->print(screen->header->textGen(screen->header));
        cursor ++;
        ...
        while( cursor < ROWS  || (line != screen->current && cursor < 3) ){
            ...
            if(line == screen->current && cursor == 2){
                lcd->print("> ");
                lcd->print(textGeneratorCurr(line));
            }
            else if(line != screen->current) lcd->print(line->textGen(line));
        ...
        if(screen->footer){
            lcd->setCursor(0, 3);
            lcd->print(screen->footer->textGen(screen->footer));
        }
    }
...
void footerUpdate(Menu* menu, valLine* var){
    if(menu && var && menu->curr->footer && menu->lcd){
